...
Note que, conforme o exemplo anterior, o trabalho de implementação da interface [AcessoBioManagerDelegate](API#acessobiolistener) é, em grande parte, a configuração dos métodos de callback. Cada método será chamado em uma situação específica de retorno de nosso SDK, como detalhado abaixo. Basta sobrescrever os métodos exemplificados no passo anterior com as lógicas de negócio de sua aplicação. 
...
Ao ser invocado, o método receberá um parâmetro do tipo `ErrorBio` que contem detalhes do erro. Saiba mais sobre o tipo `ErrorBio` no artigo de [Referências](API#objeto-errorbio) deste SDK.
